...
export const updateCommunityComment = async (
  ...
  const endpoint = `${API_ENDPOINTS.BOARD.COMMUNITY.COMMENT(
    postId
  )}/${commentId}`;

  try {
    const response = await apiUtils.put(endpoint, commentData);
    return response;
  } catch (error) {
...
// 12. 입양 게시글 댓글 조회
export const getAdoptionComments = async (postId) => {
  const endpoint = `${API_ENDPOINTS.BOARD.ADOPTION.DETAIL(postId)}/comments`;

  try {
    const response = await apiUtils.get(endpoint);
    return response;
  } catch (error) {
    console.error("입양 게시글 댓글 조회 실패:", error);
    throw error;
  }
};

// 13. 입양 게시글 댓글 작성
export const createAdoptionComment = async (postId, commentData) => {
  const endpoint = `${API_ENDPOINTS.BOARD.ADOPTION.DETAIL(postId)}/comments`;
  ...
  try {
    ...
    console.error("입양 게시글 댓글 작성 실패:", error);
    throw error;
  }
};

// 14. 입양 게시글 댓글 수정
export const updateAdoptionComment = async (postId, commentId, commentData) => {
  const endpoint = `${API_ENDPOINTS.BOARD.ADOPTION.DETAIL(
    postId
  )}/comments/${commentId}`;
  ...
  try {
    ...
    console.error("입양 게시글 댓글 수정 실패:", error);
    throw error;
  }
};

// 15. 입양 게시글 댓글 삭제
export const deleteAdoptionComment = async (postId, commentId) => {
  const endpoint = `${API_ENDPOINTS.BOARD.ADOPTION.DETAIL(
    postId
  )}/comments/${commentId}`;
  ...
  try {
    ...
    console.error("입양 게시글 댓글 삭제 실패:", error);
    throw error;
  }
};
...
// 분양게시판 목록 조회 (새로운 API)
export const getAdoptionBoardPosts = async (
  searchWord = "",
  petCategory = "",
  address = "",
  sex = "",
...
// 유기동물 목록 조회
export const getStrayAnimals = async (
  searchWord = "",
  upKindNm = "",
  careAddr = "",
  sexCode = "",
