...
const AdminPolicyDetail = () => {
  ...
  const fetchPolicyData = async () => {
    setLoading(true);
    try {
      const result = await getAdminTermsDetail("policy", id);
      setData(result);
      setFormData({
        title: result.title || "",
  ...
  const handleSubmit = async (e) => {
    ...
    // 확인 다이얼로그
    if (!window.confirm("개인정보처리방침을 수정하시겠습니까?")) {
      return;
    }
    ...
    try {
      await updateAdminTerms("policy", id, formData);
      
      alert("개인정보처리방침이 수정되었습니다.");
      
      setIsEditing(false);
      fetchPolicyData(); // 데이터 다시 불러오기
    } catch (error) {
      console.error("개인정보처리방침 수정 오류:", error);
      alert("개인정보처리방침 수정 중 오류가 발생했습니다.");
    } finally {
  ...
  const handleDeleteClick = async () => {
    // 확인 다이얼로그
    if (!window.confirm("개인정보처리방침을 삭제하시겠습니까?")) {
      return;
    }
    ...
    try {
      await deleteAdminTerms("policy", id);
      
      alert("개인정보처리방침이 삭제되었습니다.");
      
      navigate("/admin/support?tab=policy");
    } catch (error) {
      console.error("개인정보처리방침 삭제 오류:", error);
      alert("개인정보처리방침 삭제 중 오류가 발생했습니다.");
    } finally {
      setIsSubmitting(false);
    }
  };
            ...
            <div className="p-3" style={{ paddingBottom: '0px' }}>
              <div className="p-6">
                <div className="mx-auto">
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle>개인정보처리방침 상세</CardTitle>
                    </CardHeader>
                    <CardContent>
