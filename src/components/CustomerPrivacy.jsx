...
const CustomerPrivacy = () => {
  ...
  const fetchPrivacy = async (page = 0) => {
    setLoading(true);
    setError(null);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium text-gray-900">개인정보처리방침</h3>
        <button
          onClick={() => fetchPrivacy(currentPage)}
          disabled={loading}
        ...
        <div className="bg-white rounded-lg border">
          ...
          <div className="px-6 py-4 border-b bg-gray-50">
            ...
            <p className="text-sm text-gray-500 mt-1">
              총 {privacyData.totalElements}개 (페이지 {currentPage + 1} /{" "}
              {privacyData.totalPages})
            </p>
          </div>
            ...
            {privacyData.content && privacyData.content.length > 0 ? (
              privacyData.content.map((item, index) => (
                <div
                  ...
                  onClick={() => handleTermsClick(item.termsId)}
                >
          ...
          {privacyData.totalPages > 1 && (
            <div className="px-6 py-4 border-t bg-gray-50">
              <div className="flex justify-center items-center space-x-2">
                ...
                {/* 페이지 번호 버튼들 */}
                {Array.from({ length: privacyData.totalPages }, (_, i) => (
                  <button
                    key={i}
                ...
                <button
                  onClick={() => handlePageChange(currentPage + 1)}
                  disabled={currentPage === privacyData.totalPages - 1}
                  className="px-3 py-2 text-sm border rounded-md hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  다음
                </button>
